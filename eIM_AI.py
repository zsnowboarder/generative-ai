...
def generate(inst_text, prompt_text):
 resp_text = "You're missing out on experiencing the full potential of Google's most advanced AI model Gemini! The author's account currently has a low credit balance, and they've also disabled the connection to Vertex AI, limiting what this AI can showcase."
 return resp_text
...
st.write('')
st.write("Responses are generated by Gemini, Google's most advanced AI model")
...
if st.button("Generate Response", help="Generate eIM based on the input text."):
    ...
    placeholder.empty()
    placeholder.write("With this proof of concept, we demonstrate the potential of AI to significantly reduce repetitive tasks, allowing officers to spend more time on the road. By leveraging AI with a push of a button, we can seamlessly add entities and text pages with proper eIM naming conventions based on details extracted from the officer's narrative. Officers can specify their requirements in natural language, and I will ensure everything is done with proper naming conventions prior to sending the report to CPIC Transcription. Importantly, officers retain full control over the edits before finalizing them. The possibilities for enhancing efficiency and productivity are truly endless.")
